...
# user specified environment (adds /home/<user>/.local/bin and /home/<user>/bin to the beginning of the $PATH variable)
if ! [[ "$PATH" =~ "$HOME/.local/bin:$HOME/bin:" ]]
then
	PATH="$HOME/.local/bin:$HOME/bin:$PATH";
fi

# export new directories to PATH variable
export PATH;

# Uncomment the following line if you don't like systemctl's auto-paging feature:
# export SYSTEMD_PAGER=

# check to see if '/home/<user>/.bashrc.d' exists
if [[ -d ~/.bashrc.d ]]
then
	# loop through each item within '/home/<user>/.bashrc.d'
	for rc in ~/.bashrc.d/*
	do
		# if the rc variable is a file
		if [[ -f "$rc" ]]
		then
			# run it (aka 'source $rc'). Basically loads any configuration files found within '/home/<user>/.bashrc.d'
			. "$rc";
		fi
	done
	
	# unset the rc variable
	unset rc;
fi
...
NOTE: You may add the following lines if you want 'ls' to be colorized:
...
NOTE: Some more aliases to avoid making mistakes

alias rm='rm -i';
alias cp='cp -i';
alias mv='mv -i';

NOTE: Check to see if tmux is installed and a interactive terminal shell is running/exists
...
if [[ $(command -v tmux; echo $?) != 1 ]]
then
	# if PS1 variable is not empty AND if TERM variable does not equal regex AND if TERM variable does not equal regex AND if TMUX variable is empty
	if [[ -n "$PS1" ]] && ([[ ! "$TERM" =~ "screen" ]] || [[ ! "$TERM" =~ "tmux" ]]) && [[ -z "$TMUX" ]]
	then
		# if there are any tmux sessions running
		if [[ $(tmux has-session -t main 2> /dev/null; echo $?) != 0 ]]
		then
			# create a tmux sessions name 'main'
			exec tmux new-session -A -s main;
		else
			# attach to the current session name main
...
# if tmux is installed
if [[ $(command -v tmux) == '/usr/bin/tmux' ]]
then
	# check if PS1 is not empty
	if [[ -n "$PS1" ]]
	then
		# check the TERM variable and see if it is NOT set to 'screen' or 'tmux'
		if [[ ! "$TERM" =~ 'screen' ]] && [[ ! "$TERM" =~ 'tmux' ]]
		then
			# check if the TMUX variable is empty
			if [[ -z "$TMUX" ]]
			then
				# check if tmux has a session
				tmux list-sessions;
				
				# if  successful (theres a session)
				if [[ $? == 0 ]]
				then
					# check if session has a name of main
					tmux has-session -t main;

					# if successful (tmux session called main)
					if [[ $? == 0 ]]
					then
						# attach to that existing session
						exec tmux attach-session -t main;
					else
						# create new tmux session called main
						exec tmux new-session -A -s main;
					fi
				else
					# create a tmux session called main
					exec tmux new-session -A -s main;
				fi
			fi
		fi
...
# enable syntax highlighting for MAN Pages -- source: https://www.youtube.com/watch?v=ooVYAR_0ftU 
export MANPAGER="less -R --use-color -Dd+r -Du+b";

# enables syntax highlighting for MAN Pages but requires the BAT utility
...
# dircolors TRAPDOOR colorscheme
